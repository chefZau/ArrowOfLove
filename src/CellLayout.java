...
public class CellLayout implements LayoutManager, java.io.Serializable {
	...
	int cgap;
	/**
	 * This is the number of rows specified for the grid. The number of rows can be
	 * changed at any time. This should be a non negative integer, where '0' means
	 * 'any number' meaning that the number of Rows in that dimension depends on the
	 * other dimension.
	 *
	 * @serial
	 * @see #getRows()
	 ...
	 * can be changed at any time. This should be a non negative integer, where '0'
	 * means 'any number' meaning that the number of Columns in that dimension
	 ...
	 * <p>
	 * One, but not both, of <code>rows</code> and <code>cols</code> can be zero,
	 ...
	 * @exception IllegalArgumentException if the value of both <code>rows</code>
	 *                                     and <code>cols</code> is set to zero
	 */
	 ...
	 * by a constructor or by the <tt>setRows</tt> method is non-zero. In that case,
	 * the number of columns displayed in the layout is determined by the total
	 ...
	 * right insets of the target container, plus the width of half a component if
	 * there is more than one row.
	 * <p>
	 * The preferred height of a grid layout is the largest preferred height of all
	 * of the components in the container plus three quarters of the largest minimum
	 * height of all of the components in the container times the number of rows
	 * greater than one, plus the vertical padding times the number of rows minus
	 ...
	 * of the target container, plus the width of half a component if there is more
	 * than one row.
	 * <p>
	 * The minimum height of a grid layout is the largest minimum height of all of
	 * the components in the container plus three quarters of the largest minimum
	 * height of all of the components in the container times the number of rows
	 * greater than one, plus the vertical padding times the number of rows minus
	 * one, plus the top and bottom insets of the target container.
	 *
	 ...
	 * to the number of rows and columns in the layout. The container's free space
